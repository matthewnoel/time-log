...
var app = (function () {
	...
	*/
	...
	var root$1 = add_locations(from_html(`<input type="button" data-theme-toggle=""/>`), ThemeToggle[FILENAME], [[21, 0]]);
	...
	function ThemeToggle($$anchor, $$props) {
		...
		const resolveInitialTheme = () => {
			...
			return strict_equals(_a = localStorage.getItem(THEME_STORAGE_KEY), null, false) && strict_equals(_a, void 0, false) ? _a : "";
		};

		let currentTheme = tag(state(proxy(resolveInitialTheme())), 'currentTheme');
		const icon = tag(user_derived(() => strict_equals(get(currentTheme), DARK) ? "🌞" : "🌚"), 'icon');
		const ariaLabel = tag(user_derived(() => strict_equals(get(icon), "🌚") ? "Change to dark theme." : "Change to light theme."), 'ariaLabel');

		user_effect(() => {
			document.documentElement.setAttribute("data-theme", get(currentTheme));
		});

		const handleClick = () => {
			const next = strict_equals(get(currentTheme), DARK) ? LIGHT : DARK;

			localStorage.setItem(THEME_STORAGE_KEY, next);
			set(currentTheme, next, true);
		};
		...
		template_effect(() => {
			set_attribute(input, 'aria-label', get(ariaLabel));
			set_value(input, get(icon));
		});
	...
	var root_1 = add_locations(from_html(`<input type="button" value="Ibid" class="svelte-1n46o8q"/>`), App[FILENAME], [[76, 16]]);

	var root_2 = add_locations(from_html(`<h2 class="svelte-1n46o8q">Latest Log</h2> <table class="svelte-1n46o8q"><thead><tr><th colspan="1">Log Time</th><th colspan="1">Duration</th><th colspan="1">Activity</th></tr></thead><tbody><tr><td> </td><td> </td><td> </td></tr></tbody></table>`, 1), App[FILENAME], [
		[88, 4],
		[
			89,
			4,
			[
				[90, 8, [[91, 12, [[92, 16], [93, 16], [94, 16]]]]],
				[97, 8, [[98, 12, [[99, 16], [100, 16], [101, 16]]]]]
			]
		]
	]);

	var root_4 = add_locations(from_html(`<tr><td> </td><td> </td><td> </td></tr>`), App[FILENAME], [[118, 16, [[119, 20], [120, 20], [121, 20]]]]);

	var root_3 = add_locations(from_html(`<h2 class="svelte-1n46o8q">Full Summary</h2> <table class="svelte-1n46o8q"><thead><tr><th colspan="1">Log Time</th><th colspan="1">Duration</th><th colspan="1">Activity</th></tr></thead><tbody></tbody></table> <input type="button" value="New Day" class="svelte-1n46o8q"/>`, 1), App[FILENAME], [
		[107, 4],
		[
			108,
			4,
			[
				[109, 8, [[110, 12, [[111, 16], [112, 16], [113, 16]]]]],
				[116, 8]
			]
		],
		[126, 4]
	...
	var root_5 = add_locations(from_html(`<input class="floating svelte-1n46o8q" type="button" value="🖋️"/>`), App[FILENAME], [[129, 4]]);

	var root = add_locations(from_html(`<!> <h1 class="svelte-1n46o8q">Time Log</h1> <div class="form-wrapper svelte-1n46o8q"><form class="svelte-1n46o8q"><div class="svelte-1n46o8q"><label for="activity">&nbsp;Activity:</label> <br/> <input type="text" name="Activity" id="activity" class="svelte-1n46o8q"/></div> <div class="button-row svelte-1n46o8q"><!> <input type="submit" value="Log Time" class="svelte-1n46o8q"/></div></form></div> <!> <!> <!> <div id="theme-wrapper" class="svelte-1n46o8q"><!></div> <footer class="svelte-1n46o8q"><a href="https://github.com/matthewnoel/time-log" target="_blank" class="svelte-1n46o8q">The Code</a> <a href="https://raw.githubusercontent.com/matthewnoel/time-log/main/third-party-licenses.txt" target="_blank" class="svelte-1n46o8q">Third-Party Licenses</a></footer>`, 1), App[FILENAME], [
		[58, 0],
		[
			59,
			0,
			[
				[
					60,
					4,
					[[61, 8, [[62, 12], [63, 12], [64, 12]]], [74, 8, [[83, 12]]]]
				]
			]
		],
		[136, 0],
	...
	function App($$anchor, $$props) {
		...
		let isActivityInputInFocus = tag(state(false), 'isActivityInputInFocus');
		let currentData = tag(state(proxy(getActivityData())), 'currentData');
		let activityInput = tag(state(null), 'activityInput');
		const entries = tag(user_derived(() => toSortedEntries(get(currentData))), 'entries');
		const latest = tag(user_derived(() => get(entries)[get(entries).length - 1]), 'latest');
		const previous = tag(user_derived(() => get(entries)[get(entries).length - 2]), 'previous');
		const refresh = () => set(currentData, getActivityData(), true);
		...
		const submitActivity = (activityValue) => {
			...
			if (equals(get(latest), null, false) && strict_equals(normalizeActivity(get(latest).value), normalizeActivity(activityValue))) {
				window.localStorage.removeItem(get(latest).key);
			}
		...
		const handleActivityFormSubmit = (event) => {
			...
			if (submitActivity(get(value))) set(value, "");
		};

		const handleIbidClick = () => {
			if (equals(get(latest), null)) return;

			submitActivity(get(latest).value);
		};

		const handleGoToInputClick = () => strict_equals(get(activityInput), null) || strict_equals(get(activityInput), void 0) ? void 0 : get(activityInput).focus();

		const handleActivityFocus = (event) => {
		...
		add_svelte_meta(() => SvelteToast(node, {}), 'component', App, 57, 0, { componentTag: 'SvelteToast' });

		var div = sibling(node, 4);
		var form = child(div);
		var div_1 = child(form);
		var input = sibling(child(div_1), 4);
		bind_this(input, ($$value) => set(activityInput, $$value), () => get(activityInput));
...

		{
			var consequent = ($$anchor) => {
				...
				template_effect(() => set_attribute(input_1, 'aria-label', `Repeat last activity: ${get(latest).value}`));
				delegated('click', input_1, handleIbidClick);
				append($$anchor, input_1);
			};

			add_svelte_meta(
				() => if_block(node_1, ($$render) => {
					if (equals(get(latest), null, false)) $$render(consequent);
				}),
				'if',
				App,
				75,
...

		{
			var consequent_1 = ($$anchor) => {
				...
				template_effect(
					($0, $1) => {
						set_text(text, $0);
						set_text(text_1, $1);
						set_text(text_2, get(latest).value);
					},
					[
						() => formatLogTimeCell(get(latest).key),
						() => formatDurationCell(get(latest).key, get(previous).key)
					]
				);
			...
			add_svelte_meta(
				() => if_block(node_2, ($$render) => {
					if (get(entries).length > 1 && get(latest) && get(previous)) $$render(consequent_1);
				}),
				'if',
				App,
				87,
...

		{
			var consequent_2 = ($$anchor) => {
				...
				add_svelte_meta(
					() => each(tbody_1, 23, () => get(entries), (entry) => entry.key, ($$anchor, entry, i) => {
						var tr_1 = root_4();
						var td_3 = child(tr_1);
						...
						template_effect(
							($0, $1) => {
								set_text(text_3, $0);
								set_text(text_4, $1);
								set_text(text_5, get(entry).value);
							},
							[
								() => formatLogTimeCell(get(entry).key),
								() => formatDurationCell(get(entry).key, get(entries)[get(i) - 1]?.key)
							]
						);
				...
				var input_3 = sibling(table_1, 2);

				delegated('click', input_3, handleNewDayClick);
				append($$anchor, fragment_2);
			};

			add_svelte_meta(
				() => if_block(node_3, ($$render) => {
					if (get(entries).length > 0) $$render(consequent_2);
				}),
				'if',
				App,
				106,
...

		{
			var consequent_3 = ($$anchor) => {
				...
				delegated('click', input_4, handleGoToInputClick);
				append($$anchor, input_4);
			};

			add_svelte_meta(
				() => if_block(node_4, ($$render) => {
					if (!get(isActivityInputInFocus)) $$render(consequent_3);
				}),
				'if',
				App,
				128,
		...
		add_svelte_meta(() => ThemeToggle(node_5, {}), 'component', App, 137, 4, { componentTag: 'ThemeToggle' });
		template_effect(($0) => input_2.disabled = $0, [() => !get(value).trim()]);
		event('submit', form, handleActivityFormSubmit);
		event('focus', input, handleActivityFocus);
		event('blur', input, handleActivityBlur);
